...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if total == 0:
            self.stdout.write(
                self.style.WARNING("No employees (and hence no users) found.")
            )
            return

        self.stdout.write(
            self.style.NOTICE(
                f"Randomizing passwords for {total} users (by Employee.name)..."
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Completed randomization for {total} users.")
        )
