...
class PerformanceFilter(django_filters.FilterSet):
    ...
    review_date_min = django_filters.DateFilter(
        field_name="review_date", lookup_expr="gte", label="Review Date (on or after)"
    )
    review_date_max = django_filters.DateFilter(
        field_name="review_date", lookup_expr="lte", label="Review Date (on or before)"
    )

    rating_min = django_filters.NumberFilter(
        field_name="rating", lookup_expr="gte", label="Rating (greater than or equal)"
    )
    rating_max = django_filters.NumberFilter(
        field_name="rating", lookup_expr="lte", label="Rating (less than or equal)"
    )
